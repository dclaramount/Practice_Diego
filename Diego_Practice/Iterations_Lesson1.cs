...
{
    public static class Iterations_Lesson1
    {
        /*
            ...
            of length 1. The number 15 has binary representation 1111 and has no binary gaps.The number 32 has
        ...
        */
        public static int Binary_Gap(int Number)
        {
            string binnaryRep = Convert.ToString(Number, 2);
            int count = 0;
            bool open = false;
            int oldCount = 0;
            foreach(char digit in binnaryRep)
            {
                if (digit == '1' && open)
                {
                    if (count > oldCount)
